...
- **The shipment is younger than the grace window** (`ALLEGRO_ORDERS_RECONCILE_SENT_GRACE_MS`,
  ...
  would race it: Allegro's checkout-form view lags its own writes by tens of
  seconds, so a re-read moments after a *successful* push still reports
  `READY_FOR_SHIPMENT`, and the sweep would "repair" an order that was never broken
