...
`@zanreal/medusa-allegro` is on npm:
...
`.medusa/server` output its `exports` point at, so nothing needs to compile on
install. It pulls `@zanreal/medusa-admin-kit` from the registry too, for the
Catalog columns it contributes.

`main` keeps moving after a release ships (see [Releasing](#releasing)), so if you
need a fix or feature that has landed on `main` but not yet been released, install
it as a git dependency instead, pinned to a commit:
...
Pin the commit you tested against. `#main` would move under you on the next push.

Installed that way, the package compiles itself on install - `prepare` runs `medusa
plugin:build`, which turns the checked-out source into the `.medusa/server` output
its `exports` point at. pnpm 10 and newer refuse to run that script for a
...
| `ALLEGRO_STOCK_SYNC_CRON`                | Schedule for the quantity **backstop**. Default `"*/15 * * * *"`. Quantities normally reach Allegro through the event path (a sale, or `marken.stock.changed`) within seconds; this sweep repairs what that missed - see `jobs/allegro-stock-sync.ts` for the exact list.                                                                                                                                                                                       |
...
`@zanreal/medusa-allegro` is on the registry. `main` runs ahead of what npm
...
(`^0.2.0`), so a published tarball resolves entirely from npm. It used to carry a
`github:` spec, which meant anyone installing from npm still needed git and access
to GitHub; publishing admin-kit is what removed that.
...
@zanreal/medusa-allegro` never resolves to a release candidate.
...
Actions, owner `zanreal-labs`, repository `medusa-allegro`, workflow
